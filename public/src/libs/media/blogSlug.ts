/**
 * Derives a kebab-case slug from a post title. Keeps Unicode letters
...
/**
 * Validates a blog post slug. Blog slugs live under /media/, so no
 * reserved-path check is needed — only format rules.
 * @param slug The slug to validate (lowercase, kebab-case).
 * @returns Error message if invalid, undefined if valid.
 */
export function validateBlogSlug(slug: string): string | undefined {
  if (!slug) {
