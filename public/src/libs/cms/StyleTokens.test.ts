...
describe(resolveCardStyle, () => {
  it('separates card and content panel classes', () => {
    const resolved = resolveCardStyle(DEFAULT_RESPONSIVE_CARD_STYLE);

    expect(resolved.cardClass).toContain('rounded-ps-xl');
    expect(resolved.panelClass).toContain('bg-ps-green-tint');
    expect(resolved.panelClass).toContain('font-body');
    expect(resolved.panelClass).toContain('px-4');
    expect(resolved.titleClass).toContain('text-ps-h6');
    expect(resolved.titleClass).toContain('font-bold');
    expect(resolved.titleClass).toContain('md:text-ps-h4');
    expect(resolved.bodyClass).toContain('text-ps-xs');
    expect(resolved.bodyClass).toContain('font-normal');
    expect(resolved.bodyClass).toContain('md:text-ps-body');
  });

  it('maps independent responsive card overrides', () => {
    ...
    expect(resolved.panelClass).toContain('xl:text-ps-white');
    expect(resolved.panelClass).toContain('xl:px-20');
    expect(resolved.cardClass).toContain('xl:my-20');
    expect(resolved.titleClass).toContain('xl:text-ps-h2');
    expect(resolved.bodyClass).toContain('xl:text-ps-lead');
  });
});
