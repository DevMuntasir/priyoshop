...
function StepItem({
  ...
  return (
    ...
    >
      ...
      <div>
        <h3 className="m-0 font-body text-xl font-bold text-ps-white sm:text-2xl">{step.title}</h3>
        <p className="m-0 mt-2 max-w-sm font-body text-ps-sm leading-relaxed text-ps-black-50 sm:mt-3 sm:text-base">
          {step.body}
        </p>
      </div>
    </motion.li>
...
function ScrollStepsMobile(props: Pick<ScrollStepsProps, 'steps' | 'heading'>) {
  return (
    <section className="container mx-auto px-4 sm:px-6 md:hidden" aria-roledescription="scroll story">
      <StoryHeading />
      {props.heading ? <div className="mt-8 max-w-md">{props.heading}</div> : null}
          ...
          <li key={s.id ?? i} className="space-y-5">
            <div className="grid grid-cols-[auto_1fr] gap-x-4">
              ...
              <div>
                ...
                <p className="m-0 mt-2 font-body text-ps-sm leading-relaxed text-ps-black-50">
                  {s.body}
                </p>
              </div>
            </div>
...
function ScrollStepsDesktop({ steps, heading, config, className = '' }: ScrollStepsProps) {
  ...
  return (
    <section
      ref={trackRef}
      style={{ height: `${n * cfg.vhPerStep}vh` }}
      className={`hidden md:block ${className} `}
      aria-roledescription="scroll story"
    >
      <div className="sticky top-20 h-screen items-center lg:top-25 ">
        ...
        <div className="container mx-auto mt-8 grid grid-cols-1 gap-8 px-4 sm:mt-14 sm:px-6 md:grid-cols-2 md:gap-12 lg:px-8">
          {/* LEFT — heading + steps */}
          <div>
            {heading ? <div className="mb-8 max-w-md sm:mb-12">{heading}</div> : null}
            <ul className="m-0 list-none space-y-6 p-0 sm:space-y-10">
              {steps.map((s, i) => (
                <StepItem
                  key={s.id ?? i}
                  i={i}
          ...
          <motion.div
            className="relative -mt-[100px] mr-[calc(50%-50vw)] overflow-hidden rounded-l-ps-xl rounded-r-none"
