...
export const SECTION_REGISTRY: Record<SectionKey, SectionDef> = {
  ...
  awards: {
    ...
    defaultStyle: makeDefaultStyle({ align: 'center' }),
    defaultContent: {
      heading: { title: '~Awards and Recognition Moments~', titleTrail: 'Moments' },
      items: AWARDS,
    },
  ...
  heroStats: {
    ...
    defaultStyle: makeDefaultStyle({ align: 'center' }),
    defaultContent: {
      heading: { title: 'Hero stats' },
      items: [
        { value: '296', name: 'Brands', logo: '/stats/s1.svg' },
        { value: '100K+', name: 'MSMEs', logo: '/stats/s2.svg' },
        { value: '1428', name: 'Route Coverage', logo: '/stats/s3.svg' },
      ],
    },
    editor: { itemsOnly: true },
  },
...
/**
 * Normalizes legacy section content into its current editable shape.
 * @param key The section key.
 * @param content The stored section content.
 * @returns Content compatible with the current editor and validation schema.
 */
  ...
  if (key === 'ecosystems') {
    return {
      ...
      items: content.items.map((item) =>
        item.style ? { ...item, style: normalizeCardStyle(item.style) } : item,
      ),
    };
  }
