...
type CardTokenField = keyof CardStyleTokens;
...
// `(inherit)` sentinel for per-device selects: the value falls back to the smaller breakpoint.
...
const ITEM_FIELDS: Record<ItemKind, ItemFieldDef[]> = {
  ...
  ecosystem: [
    ...
    { field: 'reverse', label: 'Show image on the right', type: 'bool' },
  ],
  card: [
    { field: 'title', label: 'Title' },
    { field: 'body', label: 'Caption', type: 'textarea', full: true },
  ...
  faq: [
    ...
    { field: 'body', label: 'Answer', type: 'textarea', full: true },
  ],
  video: [
    { field: 'title', label: 'Title' },
...
const ECOSYSTEM_FIELD_GROUPS: ItemFieldGroup[] = [
  {
    title: 'Content',
  ...
  {
    title: 'Image',
    description: 'Use a public image path and provide descriptive alt text.',
    fields: ['image', 'imageAlt'],
  },
  {
    title: 'Call to action',
    fields: ['ctaLabel', 'href'],
  },
  {
    title: 'Layout',
    description: 'Alternate the image position to create a staggered layout.',
    fields: ['reverse'],
  },
];

const SLIDE_FIELD_GROUPS: ItemFieldGroup[] = [
  {
    title: 'Content',
...
function CardStyleEditor(props: {
  style?: ResponsiveCardStyle;
  device: Device;
  onDeviceChange: (device: Device) => void;
  onChange: (style: ResponsiveCardStyle) => void;
}) {
  const currentStyle = props.style ?? DEFAULT_RESPONSIVE_CARD_STYLE;

  const updateToken = <K extends CardTokenField>(
    field: K,
    value: CardStyleTokens[K] | undefined,
  ) => {
    if (props.device === 'mobile') {
      ...
      props.onChange({
        ...currentStyle,
        base: { ...currentStyle.base, [field]: value },
      });
      return;
    }

    const { [field]: _removed, ...rest } = currentStyle[props.device] ?? {};
    const layer: Partial<CardStyleTokens> =
      value === undefined ? rest : { ...rest, [field]: value };
    props.onChange({
      ...currentStyle,
      [props.device]: Object.keys(layer).length > 0 ? layer : undefined,
    });
  };
  ...
  const resetDevice = () => {
    ...
    props.onChange({ ...currentStyle, [props.device]: undefined });
  };

  const renderToken = <K extends CardTokenField>(
    label: string,
    field: K,
    options: readonly CardStyleTokens[K][],
  ) => {
    if (props.device === 'mobile') {
      return (
        <StyleSelect
          label={label}
          value={currentStyle.base[field]}
          options={options}
          onChange={(value: CardStyleTokens[K]) => {
    ...
    return (
      <DeviceStyleSelect
        label={label}
        value={currentStyle[props.device]?.[field]}
        options={options}
        onChange={(value?: CardStyleTokens[K]) => {
          updateToken(field, value);
        }}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <p className="text-xs leading-relaxed text-gray-400">
          {props.device === 'mobile'
            ? 'Mobile is the base style for this card.'
            : 'Unset controls inherit from the nearest smaller screen.'}
        </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {renderToken('Description size', 'bodySize', TITLE_SIZE_OPTIONS)}
        {renderToken('Font family', 'fontFamily', FONT_FAMILY_OPTIONS)}
        {renderToken('Title weight', 'fontWeight', FONT_WEIGHT_OPTIONS)}
        {renderToken('Description weight', 'bodyWeight', FONT_WEIGHT_OPTIONS)}
...
function ItemField(props: {
  ...
  if (isMediaField) {
    return (
      <div className={props.def.full || isMediaField ? 'col-span-2 block w-full' : 'block flex-1'}>
        <MediaInput
          label={props.def.label}
          value={typeof raw === 'string' ? raw : ''}
          onChange={(val) => props.onChange(val)}
          placeholder={`Choose or enter ${props.def.label.toLowerCase()}`}
        />
      </div>
    );
  }
...
function EcosystemItemEditor(props: {
  item: SectionItem;
  index: number;
  itemFields: ItemFieldDef[];
  device: Device;
  ...
  return (
    ...
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 bg-gray-50 px-4 py-3 hover:bg-gray-100">
        <span className="flex min-w-0 items-center gap-3">
          <span
            className="size-10 shrink-0 rounded-md border border-gray-200 bg-gray-100 bg-cover bg-center"
            style={
              props.item.image ? { backgroundImage: `url(${props.item.image})` } : undefined
            }
          />
          <span className="min-w-0">
            <span className="block text-sm font-semibold text-gray-900">
              Ecosystem {props.index + 1}
            </span>
            <span className="block truncate text-xs text-gray-500">
              {props.item.title || 'Untitled ecosystem'}
            </span>
          </span>
        </span>
        <span className="shrink-0 text-gray-400 transition-transform group-open:rotate-180">⌄</span>
      ...
      <div className="space-y-5 p-4">
        {ECOSYSTEM_FIELD_GROUPS.map((group, groupIndex) => (
          <section
            key={group.title}
            className={groupIndex === 0 ? 'space-y-3' : 'space-y-3 border-t border-gray-100 pt-5'}
          >
        ...
        <details className="group/design border-t border-gray-100 pt-5">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
            <span className="text-xs font-semibold tracking-wide text-gray-700 uppercase">
              Design
            </span>
            <span className="flex items-center gap-2 text-xs text-gray-400">
              {DEVICE_LABELS[props.device]}
              <span className="transition-transform group-open/design:rotate-180">⌄</span>
            </span>
          </summary>
          <div className="mt-3 space-y-3">
            <p className="text-xs leading-relaxed text-gray-400">
              Style this card independently for each screen size.
            </p>
            <CardStyleEditor
...
function GenericItemEditor(props: {
  ...
  const image = props.item.image || props.item.logo;
  ...
  return (
    ...
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 bg-gray-50 px-4 py-3 hover:bg-gray-100">
        <span className="flex min-w-0 items-center gap-3">
          <span
            className="size-10 shrink-0 rounded-md border border-gray-200 bg-gray-100 bg-contain bg-center bg-no-repeat"
...
function RepeatedItemEditor(props: {
  item: SectionItem;
  index: number;
  itemKind: ItemKind;
  itemFields: ItemFieldDef[];
      ...
      {hints.headingOnly ? null : (
        <>
          ...
          {props.items.map((item, index) => (
            // biome-ignore lint/suspicious/noArrayIndexKey: section items are stored positionally
            <RepeatedItemEditor
              key={index}
              item={item}
              index={index}
          ...
          <button
            type="button"
            className="w-full rounded-lg border border-dashed border-gray-300 px-4 py-3 text-sm font-medium text-gray-600 hover:border-gray-400 hover:bg-gray-50 hover:text-gray-900"
            onClick={props.onAddItem}
          >
            + Add {ITEM_KIND_LABELS[props.itemKind].toLowerCase()}
          </button>
        </>
      )}
    </>
...
export const SectionEditor = (props: { page: PageKey; sectionKey: SectionKey }) => {
  ...
  const patchItemStyle = (index: number, style: ResponsiveCardStyle) => {
    const items = content.items.map((item, i) => (i === index ? { ...item, style } : item));
    patchContent({ ...content, items });
  };
  ...
  const itemFields = ITEM_FIELDS[current.itemKind].map((field) =>
    hints.listBody && field.field === 'body'
      ? { ...field, label: 'Caption (one bullet per line)' }
      : field,
  );
          ...
          <Panel title="Design">
            ...
            {device === 'mobile' ? null : (
              <p className="text-xs text-gray-400">
                Unset controls inherit from the nearest smaller screen. Set only what changes at
                this breakpoint.
              </p>
            )}
            <div className="grid grid-cols-2 gap-3">
