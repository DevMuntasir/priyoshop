    ...
    <div className="mx-auto my-12 max-w-4xl sm:my-16 lg:my-20">
      {steps.map((item, index) => {
        ...
        return (
          <div
            key={index}
            className={`mx-auto mt-10 grid max-w-4xl grid-cols-1 items-center gap-8 sm:gap-10 lg:grid-cols-2 lg:gap-10 ${reversed ? "lg:[&>div:first-child]:order-2" : ""
              }`}
          >
            {/* Image Container */}
            <div className="relative flex w-full min-w-0 justify-center after:absolute after:bottom-0 after:left-1/2 after:h-px after:w-full after:-translate-x-1/2 after:bg-ps-white-700/50">
              <div className="w-full max-w-[430px] min-w-0 rounded-t-ps-xl border-2 border-b-0 bg-ps-white-600 p-3 sm:rounded-t-ps-hero sm:p-5">
                <Image
                  src={item.image}
