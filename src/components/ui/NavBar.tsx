...
export function NavBar({
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-999 w-full px-[env(safe-area-inset-left)]">
      ...
      <div className="fixed inset-x-3 top-[calc(env(safe-area-inset-top)+0.75rem)] mx-auto flex h-16 max-w-3xl items-center gap-2 rounded-ps-sm md:rounded-ps-md border border-ps-white-700/10 bg-ps-white/70 px-3 shadow-sm backdrop-blur-sm sm:inset-x-4 sm:h-18 sm:gap-3 sm:px-4 lg:hidden">
        <button
          type="button"
          onClick={handleLogoClick}
          className="flex shrink-0 border-none bg-transparent p-0 min-w-fit pl-3"
        >
          <Logo width={160} className="w-34 sm:w-36" />
        </button>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1 sm:gap-3">
          {secondary && (
            <button
              type="button"
              onClick={handleSecondary}
              className="hidden sm:inline-flex items-center border-none bg-transparent px-2 py-2 sm:px-3 sm:py-0 font-body text-xs sm:text-ps-sm font-semibold whitespace-nowrap text-ps-black hover:text-ps-ink-600 rounded-lg active:bg-ps-black/5 min-h-11"
            >
              {secondary}
            </button>
          )}
          {cta && (
            <Button size="md" onClick={onCta} className="hidden text-xs sm:text-sm px-3 sm:px-4 py-2">
              {cta}
            </Button>
          )}
          <button
